...
class TestPredictions(unittest.TestCase):
    ...
    def test_backend_prediction(self):
        ...
        self.assertEqual(response.json().get("predicted_genre"), "expected_prediction_backend")

    def test_vgg19_backend_prediction(self):
        response = requests.post("http://localhost:8082/vgg", json={"data": "sample_data"})
        print(response.json())
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json().get("predicted_genre"), "expected_prediction_vgg19")
